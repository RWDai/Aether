...
pub(crate) async fn execute_video_task_refresh_plan(
    ...
    match fetch_video_task_refresh_attempt(state, executor_base_url, refresh_plan).await? {
...
pub(crate) fn spawn_video_task_poller(state: AppState) -> Option<JoinHandle<()>> {
    ...
    if !state.video_tasks.is_rust_authoritative() || state.executor_base_url.is_none() {
        return None;
    }
...
async fn fetch_video_task_refresh_attempt(
    ...
    if response.status() != axum::http::StatusCode::OK {
        return Ok(VideoTaskRefreshAttempt::Error(VideoTaskRefreshError {
            message: format!("executor returned {}", response.status()),
            permanent: false,
        }));
    }
