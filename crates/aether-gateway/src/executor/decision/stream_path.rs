...
use super::super::*;
use super::plan::maybe_execute_stream_via_plan_fallback;
...
use super::{
    ...
    maybe_execute_stream_via_remote_decision,
};
...
use super::files::maybe_build_stream_local_gemini_files_decision_payload;
use super::openai_chat::maybe_build_stream_local_decision_payload;
use super::openai_cli::maybe_build_stream_local_openai_cli_decision_payload;
use super::same_format_provider::maybe_build_stream_local_same_format_provider_decision_payload;
...
pub(crate) async fn maybe_build_stream_decision_payload_via_local_path(
    ...
    if let Some(payload) = maybe_build_stream_local_decision_payload(
    ...
    if let Some(payload) = maybe_build_stream_local_openai_cli_decision_payload(
        state, parts, trace_id, decision, body_json, plan_kind,
    )
    .await?
    {
    ...
    if let Some(payload) = maybe_build_stream_local_same_format_provider_decision_payload(
        state, parts, trace_id, decision, body_json, plan_kind,
    )
    .await?
    {
    ...
    if let Some(payload) = maybe_build_stream_local_gemini_files_decision_payload(
        state, parts, trace_id, decision, plan_kind,
    )
    .await?
    {
...
pub(in crate::gateway::executor) async fn maybe_execute_via_stream_decision_path(
    state: &AppState,
...
async fn maybe_build_local_video_task_content_stream_decision_payload(
    ...
    Ok(Some(GatewayControlSyncDecisionResponse {
        action: EXECUTOR_STREAM_DECISION_ACTION.to_string(),
