...
#[tokio::test]
#[ignore = "python decision/plan fallback removed from ai hot path"]
async fn gateway_finalizes_openai_chat_cross_format_sync_json_errors_without_control_execute() {
    #[derive(Debug, Clone)]
    struct SeenReportSyncRequest {
        trace_id: String,
        report_kind: String,
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "executor_candidate": true,
                    "auth_context": {
                        "user_id": "user-chat-cross-error-123",
                        "api_key_id": "key-chat-cross-error-123",
                        "access_allowed": true
                    },
                    "public_path": "/v1/chat/completions"
                }))
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan_kind": "openai_chat_sync",
                    "plan": {
                        "request_id": "req-openai-chat-cross-error-123",
                        "provider_name": "gemini",
                        "provider_id": "provider-openai-chat-cross-error-123",
                        "endpoint_id": "endpoint-openai-chat-cross-error-123",
                        "key_id": "key-openai-chat-cross-error-123",
                        "method": "POST",
    ...
    let response = reqwest::Client::new()
        .post(format!("{gateway_url}/v1/chat/completions"))
        .header(http::header::CONTENT_TYPE, "application/json")
        .header(TRACE_ID_HEADER, "trace-openai-chat-cross-error-123")
        .body("{\"model\":\"gpt-5\",\"messages\":[]}")
        .send()
        .await
        .expect("request should succeed");
    ...
    assert_eq!(
        seen_report_request.trace_id,
        "trace-openai-chat-cross-error-123"
    );
    assert_eq!(seen_report_request.report_kind, "openai_chat_sync_error");
    assert_eq!(seen_report_request.status_code, 429);
    assert_eq!(seen_report_request.error_message, "rate limited");
...
#[tokio::test]
#[ignore = "python decision/plan fallback removed from ai hot path"]
async fn gateway_finalizes_openai_chat_direct_sync_text_errors_without_control_execute() {
    #[derive(Debug, Clone)]
    struct SeenReportSyncRequest {
        report_kind: String,
        status_code: u64,
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "executor_candidate": true,
                    "auth_context": {
                        "user_id": "user-chat-text-error-123",
                        "api_key_id": "key-chat-text-error-123",
                        "access_allowed": true
                    },
                    "public_path": "/v1/chat/completions"
                }))
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan_kind": "openai_chat_sync",
                    "plan": {
                        "request_id": "req-openai-chat-text-error-123",
                        "provider_name": "openai",
                        "provider_id": "provider-openai-chat-text-error-123",
                        "endpoint_id": "endpoint-openai-chat-text-error-123",
                        "key_id": "key-openai-chat-text-error-123",
                        "method": "POST",
                        "url": "https://api.openai.example/v1/chat/completions",
                        "headers": {
                            "authorization": "Bearer upstream-key",
                    ...
                    *report_hits_inner.lock().expect("mutex should lock") += 1;
                    let body_text = payload
                        ...
                        .map(|value| {
                            ...
                            .expect("body bytes should be utf-8")
                        })
                        .unwrap_or_default();
                    *seen_report_inner.lock().expect("mutex should lock") =
                        Some(SeenReportSyncRequest {
    ...
    let executor = Router::new().route(
        ...
        any(|_request: Request| async move {
            Json(json!({
                ...
                "body": {
                    "body_bytes_b64": base64::engine::general_purpose::STANDARD.encode("rate limited raw")
                }
            }))
        }),
    );
    ...
    let response = reqwest::Client::new()
        .post(format!("{gateway_url}/v1/chat/completions"))
        .header(http::header::CONTENT_TYPE, "application/json")
        .header(TRACE_ID_HEADER, "trace-openai-chat-text-error-123")
        .body("{\"model\":\"gpt-5\",\"messages\":[]}")
        .send()
        .await
        .expect("request should succeed");
    ...
    assert!(
        ...
        "content-type should stay text/plain"
    );
    assert_eq!(
        response.text().await.expect("body should read"),
        "rate limited raw"
    ...
    assert_eq!(seen_report_request.report_kind, "openai_chat_sync_error");
    assert_eq!(seen_report_request.status_code, 429);
    assert_eq!(seen_report_request.body_text, "rate limited raw");
