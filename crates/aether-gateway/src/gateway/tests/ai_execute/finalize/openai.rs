...
#[tokio::test]
async fn gateway_executes_openai_chat_same_format_sync_via_local_finalize_response() {
    #[derive(Debug, Clone)]
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "auth_context": {
                        "user_id": "user-openai-chat-sync-local-123",
                        "api_key_id": "key-openai-chat-sync-local-123",
                        "access_allowed": true
                    },
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan": {
                        "request_id": "req-openai-chat-sync-local-123",
                        "provider_name": "openai",
                        "provider_id": "provider-openai-chat-sync-local-123",
                        "endpoint_id": "endpoint-openai-chat-sync-local-123",
                        "key_id": "key-openai-chat-sync-local-123",
                        "method": "POST",
                ...
                async move {
                    *seen_finalize_inner.lock().expect("mutex should lock") =
                    ...
                    let mut response = Response::builder()
                        ...
                        .body(Body::from("{\"id\":\"ignored-finalize\"}"))
    ...
    let executor = Router::new().route(
        ...
        any(|_request: Request| async move {
            Json(json!({
                ...
                "body": {
                    "json_body": {
                        "id": "chatcmpl-openai-sync-local-123",
    ...
    let response = reqwest::Client::new()
        ...
        .header(TRACE_ID_HEADER, "trace-openai-chat-sync-local-123")
    ...
    assert!(
        ...
        "finalize-sync should not be called for same-format openai chat sync"
    );
