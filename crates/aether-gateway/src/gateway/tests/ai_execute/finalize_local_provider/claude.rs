...
async fn gateway_executes_claude_chat_sync_same_format_via_local_finalize_response() {
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "auth_context": {
                        "user_id": "user-claude-chat-sync-local-123",
                        "api_key_id": "key-claude-chat-sync-local-123",
                        "access_allowed": true
                    },
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan": {
                        "request_id": "req-claude-chat-sync-local-123",
                        "candidate_id": "cand-claude-chat-sync-local-123",
                        "provider_name": "claude",
                        "provider_id": "provider-claude-chat-sync-local-123",
                        "endpoint_id": "endpoint-claude-chat-sync-local-123",
                        "key_id": "key-claude-chat-sync-local-123",
                        "method": "POST",
                ...
                async move {
                    *seen_finalize_inner.lock().expect("mutex should lock") =
                    ...
                    let mut response = Response::builder()
                        ...
                        .body(Body::from("{\"id\":\"ignored-claude-finalize\"}"))
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-claude-chat-sync-local-123")
            .await
    ...
    assert!(
        ...
        "finalize-sync should not be called for same-format claude chat sync"
    );
...
async fn gateway_executes_claude_chat_sync_upstream_stream_via_local_finalize_response() {
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-claude-chat-stream-sync-direct-123")
            .await
            .expect("request candidate trace should read");
        if stored_candidates.len() == 1
            && stored_candidates[0].status == RequestCandidateStatus::Success
...
async fn gateway_executes_claude_cli_sync_upstream_stream_via_local_finalize_response() {
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-claude-cli-stream-sync-direct-123")
            .await
            .expect("request candidate trace should read");
        if stored_candidates.len() == 1
            && stored_candidates[0].status == RequestCandidateStatus::Success
