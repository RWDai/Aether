...
async fn gateway_executes_gemini_chat_sync_same_format_via_local_finalize_response() {
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "auth_context": {
                        "user_id": "user-gemini-chat-sync-local-123",
                        "api_key_id": "key-gemini-chat-sync-local-123",
                        "access_allowed": true
                    },
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan": {
                        "request_id": "req-gemini-chat-sync-local-123",
                        "candidate_id": "cand-gemini-chat-sync-local-123",
                        "provider_name": "gemini",
                        "provider_id": "provider-gemini-chat-sync-local-123",
                        "endpoint_id": "endpoint-gemini-chat-sync-local-123",
                        "key_id": "key-gemini-chat-sync-local-123",
                        "method": "POST",
                        "url": "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro-upstream:generateContent",
                        "headers": {
                ...
                async move {
                    *seen_finalize_inner.lock().expect("mutex should lock") =
                    ...
                    let mut response = Response::builder()
                        ...
                        .body(Body::from("{\"responseId\":\"ignored-gemini-finalize\"}"))
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-gemini-chat-sync-local-123")
            .await
    ...
    assert!(
        ...
        "finalize-sync should not be called for same-format gemini chat sync"
    );
...
async fn gateway_executes_gemini_chat_sync_upstream_stream_via_local_finalize_response() {
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan": {
                        ...
                        "method": "POST",
                        "url": "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro-upstream:streamGenerateContent?alt=sse",
                        "headers": {
                            "content-type": "application/json",
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-gemini-chat-stream-sync-direct-123")
            .await
            .expect("request candidate trace should read");
        if stored_candidates.len() == 1
            && stored_candidates[0].status == RequestCandidateStatus::Success
...
async fn gateway_executes_gemini_cli_sync_upstream_stream_via_local_finalize_response() {
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan": {
                        ...
                        "method": "POST",
                        "url": "https://generativelanguage.googleapis.com/v1beta/models/gemini-cli-upstream:streamGenerateContent?alt=sse",
                        "headers": {
                            "content-type": "application/json",
                            "accept": "text/event-stream",
                            "x-goog-api-key": "upstream-secret"
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-gemini-cli-stream-sync-direct-123")
            .await
            .expect("request candidate trace should read");
        if stored_candidates.len() == 1
            && stored_candidates[0].status == RequestCandidateStatus::Success
...
async fn gateway_executes_antigravity_gemini_cli_sync_upstream_stream_via_local_finalize_response()
{
    ...
    for _ in 0..50 {
        stored_candidates = request_candidate_repository
            .list_by_request_id("req-antigravity-cli-stream-sync-direct-123")
            .await
            .expect("request candidate trace should read");
        if stored_candidates.len() == 1
            && stored_candidates[0].status == RequestCandidateStatus::Success
