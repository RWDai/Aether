...
#[tokio::test]
#[ignore = "python decision/plan fallback removed from ai hot path"]
async fn gateway_finalizes_openai_chat_stream_text_errors_without_control_execute() {
    #[derive(Debug, Clone)]
    struct SeenReportSyncRequest {
        report_kind: String,
        status_code: u64,
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "executor_candidate": true,
                    "auth_context": {
                        "user_id": "user-openai-chat-stream-text-error-123",
                        "api_key_id": "key-openai-chat-stream-text-error-123",
                        "access_allowed": true
                    },
                    "public_path": "/v1/chat/completions"
                }))
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "plan_kind": "openai_chat_stream",
                    "plan": {
                        "request_id": "req-openai-chat-stream-text-error-123",
                        "provider_name": "openai",
                        "provider_id": "provider-openai-chat-stream-text-error-123",
                        "endpoint_id": "endpoint-openai-chat-stream-text-error-123",
                        "key_id": "key-openai-chat-stream-text-error-123",
                        "method": "POST",
                        "url": "https://api.openai.example/v1/chat/completions",
                        "headers": {
                            "authorization": "Bearer upstream-key",
    ...
    let executor = Router::new().route(
        ...
        any(|_request: Request| async move {
            let frames = concat!(
                "{\"type\":\"headers\",\"payload\":{\"kind\":\"headers\",\"status_code\":429,\"headers\":{\"content-type\":\"text/plain; charset=utf-8\"}}}\n",
                "{\"type\":\"data\",\"payload\":{\"kind\":\"data\",\"text\":\"rate limited raw\"}}\n",
                "{\"type\":\"eof\",\"payload\":{\"kind\":\"eof\"}}\n"
            );
            let mut response = Response::builder()
                .status(StatusCode::OK)
    ...
    let response = reqwest::Client::new()
        .post(format!("{gateway_url}/v1/chat/completions"))
        .header(http::header::CONTENT_TYPE, "application/json")
        .header(TRACE_ID_HEADER, "trace-openai-chat-stream-text-error-123")
        .body("{\"model\":\"gpt-5\",\"messages\":[],\"stream\":true}")
        .send()
        .await
        .expect("request should succeed");
