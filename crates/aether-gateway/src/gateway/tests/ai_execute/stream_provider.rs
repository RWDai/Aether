...
async fn gateway_executes_kiro_claude_cli_stream_via_local_provider_catalog_candidate() {
    ...
    assert_eq!(
        seen_executor_request.accept,
        "application/vnd.amazon.eventstream"
    );
    assert_eq!(seen_executor_request.host, "q.us-east-1.amazonaws.com");
    assert_eq!(seen_executor_request.endpoint_tag, "kiro-cli-local-stream");
    assert_eq!(
        seen_executor_request.mapped_model,
        "claude-sonnet-4-upstream"
    );
    assert_eq!(seen_executor_request.current_content, "hello stream");
    assert_eq!(seen_executor_request.debug_tag, "kiro-local-stream");
    assert_eq!(
...
async fn gateway_executes_claude_cli_stream_via_local_decision_gate_without_python_decision_stream()
{
    ...
    assert_eq!(seen_executor_request.endpoint_tag, "claude-cli-local");
    assert_eq!(seen_executor_request.metadata_mode, "safe");
    assert_eq!(seen_executor_request.metadata_source, "desktop-claude-cli");
    assert_eq!(
        seen_executor_request.proxy_node_id,
...
async fn gateway_executes_claude_code_cli_stream_via_local_decision_gate_without_python_decision_stream(
    ...
    assert_eq!(seen_executor_request.anthropic_version, "2023-06-01");
    assert_eq!(
        seen_executor_request.anthropic_beta,
        "claude-code-20250219,oauth-2025-04-20,interleaved-thinking-2025-05-14,custom-beta"
    );
    assert_eq!(seen_executor_request.x_app, "cli");
    assert_eq!(seen_executor_request.x_stainless_helper_method, "stream");
    assert_eq!(seen_executor_request.user_agent, "Claude-Code/9.9");
    assert_eq!(seen_executor_request.endpoint_tag, "claude-code-cli-local");
    assert_eq!(seen_executor_request.metadata_mode, "safe");
    assert_eq!(
    ...
    assert_eq!(
        seen_executor_request.assistant_content,
        json!([
            {"type":"thinking","thinking":"keep","signature":"sig_valid"},
            {"type":"redacted_thinking","data":"keep-redacted","signature":"sig_redacted"},
            {"type":"text","text":"ok"}
    ...
    assert_eq!(seen_executor_request.tls_profile, "claude_code_nodejs");
