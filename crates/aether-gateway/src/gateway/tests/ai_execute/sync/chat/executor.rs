...
#[tokio::test]
async fn gateway_uses_python_execute_emergency_for_openai_chat_sync_when_opted_in_after_executor_miss(
) {
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "auth_context": {
                        "user_id": "user-openai-chat-emergency-ignore-123",
                        "api_key_id": "key-openai-chat-emergency-ignore-123",
                        "access_allowed": true
                    },
    ...
    let response = reqwest::Client::new()
        ...
        .header(TRACE_ID_HEADER, "trace-openai-chat-emergency-ignore-123")
        .body("{\"model\":\"gpt-5\",\"messages\":[]}")
        .send()
        .await
        .expect("request should succeed");
