...
#[tokio::test]
async fn gateway_executes_openai_chat_sync_via_local_decision_gate_without_python_decision_sync() {
    #[derive(Debug, Clone)]
        ...
        any(move |request: Request| {
            ...
            async move {
                ...
                *seen_executor_inner.lock().expect("mutex should lock") =
                ...
                Json(json!({
                    "request_id": "trace-openai-chat-local-123",
                    "status_code": 200,
                    "headers": {
...
async fn gateway_executes_openai_chat_sync_with_custom_path_via_local_decision_gate_without_python_decision_sync(
    ...
    assert_eq!(seen_executor_request.endpoint_tag, "openai-chat-local");
    assert_eq!(seen_executor_request.renamed_header, "rename-openai-chat");
    assert!(!seen_executor_request.dropped_header_present);
    assert_eq!(seen_executor_request.metadata_mode, "safe");
    assert_eq!(seen_executor_request.metadata_source, "desktop-openai");
