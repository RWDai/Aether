...
async fn gateway_executes_gemini_cli_sync_via_local_decision_gate_without_python_decision_sync() {
    ...
    assert_eq!(seen_executor_request.endpoint_tag, "gemini-cli-local");
    assert_eq!(seen_executor_request.metadata_mode, "safe");
    assert_eq!(seen_executor_request.metadata_source, "desktop-gemini-cli");
    assert!(!seen_executor_request.tool_config_present);
    assert_eq!(
...
async fn gateway_executes_gemini_cli_sync_via_local_decision_gate_after_oauth_refresh() {
    ...
    assert!((seen_executor_request.exact_temperature - 0.2).abs() < f64::EPSILON);
    assert_eq!(seen_executor_request.endpoint_tag, "gemini-cli-oauth-local");
    assert_eq!(seen_executor_request.metadata_mode, "safe");
    assert_eq!(seen_executor_request.metadata_source, "desktop-gemini-cli");
    assert!(!seen_executor_request.tool_config_present);
    assert_eq!(
...
async fn gateway_executes_vertex_ai_gemini_cli_sync_via_local_decision_gate_without_python_decision_sync(
    ...
    assert!(seen_executor_request.authorization.is_empty());
    assert!(seen_executor_request.x_goog_api_key.is_empty());
    assert_eq!(seen_executor_request.user_agent, "GeminiCLI/1.0");
    assert!(!seen_executor_request.has_model_field);
    assert!((seen_executor_request.exact_temperature - 0.2).abs() < f64::EPSILON);
    assert_eq!(seen_executor_request.endpoint_tag, "vertex-cli-local");
    assert_eq!(seen_executor_request.metadata_mode, "safe");
    assert_eq!(seen_executor_request.metadata_source, "desktop-vertex-cli");
    assert!(!seen_executor_request.tool_config_present);
...
async fn gateway_executes_antigravity_gemini_cli_sync_via_local_decision_gate_after_oauth_refresh()
{
    ...
    assert_eq!(seen_executor_request.x_client_name, "antigravity");
    assert_eq!(seen_executor_request.x_client_version, "1.2.3");
    assert_eq!(
        seen_executor_request.x_vscode_sessionid,
        "sess-antigravity-local-123"
    );
    assert_eq!(
        seen_executor_request.x_goog_api_client,
        "gl-node/18.18.2 fire/0.8.6 grpc/1.10.x"
    );
    assert_eq!(seen_executor_request.project, "project-antigravity-local-1");
    assert_eq!(
        seen_executor_request.request_id,
        "trace-antigravity-cli-oauth-local-sync-123"
    );
    assert_eq!(seen_executor_request.model, "claude-sonnet-4-5");
    assert_eq!(seen_executor_request.user_agent, "antigravity");
    assert_eq!(seen_executor_request.request_type, "agent");
    assert_eq!(seen_executor_request.contents_len, 0);
    assert!((seen_executor_request.exact_temperature - 0.2).abs() < f64::EPSILON);
    assert!(!seen_executor_request.request_has_model);
