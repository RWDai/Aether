...
async fn assert_adaptive_route_returns_local_503(
    data_state: crate::gateway::data::GatewayDataState,
    method: http::Method,
    path: &str,
...
async fn gateway_handles_admin_adaptive_mode_returns_maintenance_without_writer() {
    ...
    assert_adaptive_route_returns_local_503(
        crate::gateway::data::GatewayDataState::with_provider_catalog_reader_for_tests(repository),
        http::Method::PATCH,
...
async fn gateway_toggles_admin_adaptive_mode_locally_with_trusted_admin_principal() {
    ...
    let gateway = build_router_with_state(
        AppState::new("http://127.0.0.1:9".to_string(), None)
            .expect("gateway should build")
            .with_data_state_for_tests(
