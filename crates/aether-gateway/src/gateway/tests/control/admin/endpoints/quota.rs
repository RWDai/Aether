...
async fn gateway_refreshes_admin_provider_quota_locally_for_codex_with_trusted_admin_principal() {
    ...
    let seen_executor = Arc::new(Mutex::new(None::<SeenExecutorRequest>));
    ...
    let gateway = build_router_with_state(
        AppState::new_with_executor(upstream_url.clone(), None, Some(executor_url.clone()))
    ...
    assert_eq!(seen_executor_request.authorization, "Bearer sk-codex-123");
    assert_eq!(seen_executor_request.provider_api_format, "openai:cli");
...
async fn gateway_refreshes_admin_provider_quota_locally_for_kiro_with_trusted_admin_principal() {
    ...
    assert_eq!(seen_executor_request.provider_api_format, "kiro:usage");
...
async fn gateway_refreshes_admin_provider_quota_locally_for_antigravity_with_trusted_admin_principal(
    ...
    assert_eq!(seen_executor_request.authorization, "Bearer ya29.ant-token");
    assert_eq!(
        seen_executor_request.provider_api_format,
        "antigravity:fetch_available_models"
    );
    assert_eq!(
        seen_executor_request.request_body,
