...
async fn gateway_uploads_admin_gemini_file_locally_with_trusted_admin_principal() {
    ...
    let seen_executor = Arc::new(Mutex::new(Vec::<SeenAdminGeminiUploadExecution>::new()));
        ...
        any(move |request: Request| {
            ...
            async move {
                ...
                seen_executor_inner.lock().expect("mutex should lock").push(
                    SeenAdminGeminiUploadExecution {
                        key_id: key_id.clone(),
                        method: payload
                            .get("method")
                            .and_then(|value| value.as_str())
    ...
    let gateway = build_router_with_state(
        AppState::new_with_executor(upstream_url.clone(), None, Some(executor_url))
    ...
    let seen_requests = seen_executor.lock().expect("mutex should lock").clone();
