...
async fn gateway_handles_admin_usage_aggregation_stats_locally_with_bearer_admin_session() {
    ...
    let state = AppState::new(upstream_url.clone(), None)
        .expect("gateway should build")
        .with_data_state_for_tests(GatewayDataState::with_usage_reader_for_tests(
            usage_repository,
        ));
...
async fn gateway_handles_admin_usage_detail_locally_with_trusted_admin_principal() {
    ...
    assert_eq!(
        payload["metadata"]["original_request_body_available"],
        false
    );
    assert_eq!(payload["has_request_body"], true);
    assert_eq!(payload["has_provider_request_body"], false);
    assert_eq!(payload["has_response_body"], false);
    assert_eq!(payload["has_client_response_body"], false);
...
async fn gateway_handles_admin_usage_curl_locally_with_trusted_admin_principal() {
    ...
    assert_eq!(payload["body"]["model"], "gpt-5");
    assert_eq!(payload["body"]["target_model"], "gpt-5-target");
    assert_eq!(payload["body"]["request_type"], "chat");
    assert_eq!(payload["body"]["api_format"], "openai:chat");
    ...
    assert!(curl.contains("\"model\":\"gpt-5\""));
    assert!(curl.contains("\"target_model\":\"gpt-5-target\""));
    assert!(curl.contains("\"request_type\":\"chat\""));
    assert!(curl.contains("\"api_format\":\"openai:chat\""));
