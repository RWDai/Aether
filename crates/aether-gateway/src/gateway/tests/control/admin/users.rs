...
async fn gateway_returns_conflict_for_admin_create_user_api_key_when_writer_unavailable() {
    ...
    let gateway = build_router_with_state(
        AppState::new(upstream_url.clone(), None)
            .expect("gateway should build")
            .with_data_state_for_tests(
                crate::gateway::data::GatewayDataState::with_auth_api_key_reader_for_tests(
                    auth_repository,
                )
                .with_user_reader(user_repository),
            ),
...
async fn gateway_returns_conflict_for_admin_create_user_when_writer_unavailable() {
    ...
    let (upstream_url, upstream_handle) = start_server(upstream).await;
    let mut state = AppState::new(upstream_url.clone(), None).expect("gateway should build");
    state = state.with_data_state_for_tests(
        crate::gateway::data::GatewayDataState::with_user_reader_for_tests(Arc::new(
...
async fn gateway_returns_conflict_for_admin_update_user_when_writer_unavailable() {
    ...
    let (upstream_url, upstream_handle) = start_server(upstream).await;
    let mut state = AppState::new(upstream_url.clone(), None)
        .expect("gateway should build")
        .with_data_state_for_tests(
            crate::gateway::data::GatewayDataState::with_user_reader_for_tests(user_repository),
        );
