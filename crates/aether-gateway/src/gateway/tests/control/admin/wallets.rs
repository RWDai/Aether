...
async fn assert_admin_wallets_route_returns_local_503(path: &str) {
    ...
    let gateway = build_router_with_state(
        AppState::new(upstream_url.clone(), None).expect("gateway should build"),
    );
...
async fn gateway_handles_admin_wallets_list_locally_with_bearer_admin_session() {
    ...
    let (upstream_url, upstream_handle) = start_server(upstream).await;
    let state = AppState::new(upstream_url.clone(), None).expect("gateway should build");
...
async fn gateway_handles_admin_api_key_wallet_detail_locally_with_trusted_admin_principal() {
    ...
    let gateway = build_router_with_state(
        AppState::new(upstream_url.clone(), None)
            .expect("gateway should build")
            .with_data_state_for_tests(
                crate::gateway::data::GatewayDataState::with_auth_and_wallet_for_tests(
                    auth_repository,
                    wallet_repository,
                ),
            ),
