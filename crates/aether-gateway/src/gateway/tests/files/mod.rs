...
#[tokio::test]
async fn gateway_uses_gemini_files_download_control_sync_even_with_legacy_headers() {
...
#[tokio::test]
async fn gateway_uses_gemini_files_download_control_sync_without_legacy_header() {
    let execute_hits = Arc::new(Mutex::new(0usize));
    let execute_hits_clone = Arc::clone(&execute_hits);
    let public_hits = Arc::new(Mutex::new(0usize));
    let public_hits_clone = Arc::clone(&public_hits);
