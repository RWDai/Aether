...
fn sample_endpoint(provider_id: &str, endpoint_id: &str) -> StoredProviderCatalogEndpoint {
...
async fn gateway_model_fetch_updates_key_and_syncs_provider_model_whitelist_associations() {
    ...
    let executor = Router::new().route(
        ...
        any(move |Json(plan): Json<ExecutionPlan>| {
            ...
            async move {
                *seen_plan.lock().expect("mutex should lock") = Some(plan);
                Json(json!({
    ...
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        ...
        vec![sample_endpoint("provider-openai", "endpoint-openai")],
    ...
    let seen_plan = seen_plan
        .lock()
        .expect("mutex should lock")
        .clone()
        .expect("executor plan should be captured");
...
async fn gateway_background_model_fetch_updates_key_and_syncs_provider_model_whitelist_associations(
    ...
    let updated_key = tokio::time::timeout(Duration::from_secs(1), async {
        loop {
            let updated_key = provider_catalog_repository
                .list_keys_by_ids(&["key-openai".to_string()])
                .await
