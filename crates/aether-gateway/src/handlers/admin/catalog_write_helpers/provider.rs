async fn build_admin_update_provider_record(
    state: &AppState,
...
async fn build_admin_create_provider_record(
    state: &AppState,
    ...
    let existing_providers = state
        ...
        .await
        .map_err(|err| format!("{err:?}"))?;
    if existing_providers.iter().any(|provider| provider.name == name) {
...
fn build_admin_fixed_provider_endpoint_record(
