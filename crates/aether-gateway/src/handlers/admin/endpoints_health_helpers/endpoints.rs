async fn build_admin_provider_endpoints_payload(
    state: &AppState,
    provider_id: &str,
    skip: usize,
    limit: usize,
...
async fn build_admin_endpoint_payload(
    state: &AppState,
    endpoint_id: &str,
) -> Option<serde_json::Value> {
    if !state.has_provider_catalog_data_reader() {
...
async fn build_admin_create_provider_endpoint_record(
    state: &AppState,
...
async fn build_admin_update_provider_endpoint_record(
    state: &AppState,
    provider: &StoredProviderCatalogProvider,
