async fn build_admin_key_health_payload(
    state: &AppState,
    key_id: &str,
...
async fn build_admin_key_rpm_payload(state: &AppState, key_id: &str) -> Option<serde_json::Value> {
    if !state.has_provider_catalog_data_reader() {
        return None;
    }
...
async fn recover_admin_key_health(
    state: &AppState,
    key_id: &str,
    api_format: Option<&str>,
) -> Option<serde_json::Value> {
...
async fn recover_all_admin_key_health(state: &AppState) -> Option<serde_json::Value> {
    if !state.has_provider_catalog_data_reader() {
        return None;
    }
