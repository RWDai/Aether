const ADMIN_LDAP_RUST_BACKEND_DETAIL: &str = "Admin LDAP routes require Rust maintenance backend";
const ADMIN_LDAP_TEST_FAILURE_MESSAGE: &str = "连接失败，请检查服务器地址、端口和凭据";

#[derive(Debug, Deserialize)]
struct AdminLdapConfigUpdateRequest {
    server_url: String,
    bind_dn: String,
    #[serde(default)]
...
struct AdminLdapConfigTestRequest {
    #[serde(default)]
...
#[derive(Debug, Clone)]
struct AdminLdapConnectionTestConfig {
    server_url: String,
    bind_dn: String,
...
async fn maybe_build_local_admin_ldap_response(
    state: &AppState,
    ...
    match decision.route_kind.as_deref() {
                ...
                && is_admin_ldap_config_root(&request_context.request_path) =>
        {
            ...
            return Ok(Some(Json(json!({ "message": "LDAP配置更新成功" })).into_response()));
        }
                ...
                && is_admin_ldap_test_root(&request_context.request_path) =>
        {
            let payload = match request_body {
                Some(body) if !body.is_empty() => {
                    match serde_json::from_slice::<AdminLdapConfigTestRequest>(body) {
                        Ok(payload) => payload,
                        Err(_) => {
                            return Ok(Some(admin_ldap_bad_request_response("请求数据验证失败")));
                        }
...
fn admin_ldap_default_search_filter() -> String {
    "(uid={username})".to_string()
}

fn admin_ldap_default_username_attr() -> String {
    "uid".to_string()
}

fn admin_ldap_default_email_attr() -> String {
    "mail".to_string()
}

fn admin_ldap_default_display_name_attr() -> String {
    "cn".to_string()
}

fn admin_ldap_default_connect_timeout() -> i32 {
    10
}

fn is_admin_ldap_config_root(request_path: &str) -> bool {
    matches!(request_path, "/api/admin/ldap/config" | "/api/admin/ldap/config/")
}

fn is_admin_ldap_test_root(request_path: &str) -> bool {
    matches!(request_path, "/api/admin/ldap/test" | "/api/admin/ldap/test/")
}

fn build_admin_ldap_config_payload(
...
async fn build_admin_ldap_update_config(
    state: &AppState,
    ...
    let user_search_filter = admin_ldap_trim_required(
    ...
    Ok(aether_data::repository::auth_modules::StoredLdapModuleConfig {
        server_url,
        bind_dn,
        bind_password_encrypted,
        base_dn,
...
async fn build_admin_ldap_test_config(
    state: &AppState,
    ...
    let mut base_dn = saved
        ...
        .filter(|value: &String| !value.is_empty());
    let mut use_starttls = saved.as_ref().map(|config| config.use_starttls).unwrap_or(false);
    ...
    if server_url.as_deref().is_none() {
        missing.push("server_url");
    }
    if bind_dn.as_deref().is_none() {
        missing.push("bind_dn");
    }
    if base_dn.as_deref().is_none() {
        missing.push("base_dn");
    }
    if bind_password.as_deref().is_none() {
...
async fn admin_ldap_test_connection(
...
fn admin_ldap_test_connection_blocking(
    ...
    let Some(server_url) = admin_ldap_normalize_server_url(&config.server_url) else {
        return (false, ADMIN_LDAP_TEST_FAILURE_MESSAGE.to_string());
    };
    let _ = &config.base_dn;
...
fn admin_ldap_normalize_server_url(server_url: &str) -> Option<String> {
...
fn admin_ldap_bad_request_response(detail: impl Into<String>) -> Response<Body> {
    (
...
fn admin_ldap_unavailable_response() -> Response<Body> {
    (
