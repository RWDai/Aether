...
fn build_admin_global_model_response(
    global_model: &StoredAdminGlobalModel,
    provider_models: &[StoredAdminProviderModel],
    now_unix_secs: u64,
) -> serde_json::Value {
...
async fn build_admin_global_models_payload(
    state: &AppState,
    skip: usize,
    limit: usize,
    ...
    models.sort_by(|left, right| left.name.cmp(&right.name).then_with(|| left.id.cmp(&right.id)));
...
async fn build_admin_global_model_payload(
    state: &AppState,
    global_model_id: &str,
) -> Option<serde_json::Value> {
    if !state.has_global_model_data_reader() {
    ...
    let model = state.get_admin_global_model_by_id(global_model_id).await.ok()??;
    let provider_models = state
        .list_admin_provider_models_by_global_model_id(&model.id)
        .await
        .ok()
...
async fn build_admin_global_model_providers_payload(
    state: &AppState,
    global_model_id: &str,
) -> Option<serde_json::Value> {
    ...
    let global_model = state.get_admin_global_model_by_id(global_model_id).await.ok()??;
    let provider_models = state
    ...
    providers.sort_by(|left, right| {
        left.get("provider_name")
            ...
            .cmp(&right.get("provider_name").and_then(serde_json::Value::as_str))
    });
...
async fn build_admin_model_catalog_payload(state: &AppState) -> Option<serde_json::Value> {
    if !state.has_global_model_data_reader() || !state.has_provider_catalog_data_reader() {
        return None;
    }
