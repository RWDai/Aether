async fn build_admin_global_model_create_record(
    state: &AppState,
    ...
    let default_price_per_request =
        normalize_optional_price(payload.default_price_per_request, "default_price_per_request")?;
...
async fn build_admin_global_model_update_record(
    _state: &AppState,
...
async fn build_admin_provider_model_create_record(
    state: &AppState,
    provider_id: &str,
...
async fn build_admin_provider_model_update_record(
    state: &AppState,
...
async fn build_admin_provider_available_source_models_payload(
    state: &AppState,
    provider_id: &str,
    ...
    payload_models.sort_by(|left, right| {
        left.get("global_model_name")
            ...
            .cmp(&right.get("global_model_name").and_then(serde_json::Value::as_str))
    });
    Some(json!({
...
async fn build_admin_batch_assign_global_models_payload(
    state: &AppState,
    provider_id: &str,
    ...
    for global_model_id in global_model_ids {
        ...
        let global_model = match resolve_admin_global_model_by_id_or_err(state, &global_model_id).await
        {
            Ok(model) => model,
            Err(detail) => {
                errors.push(json!({
...
async fn build_admin_import_provider_models_payload(
    state: &AppState,
    provider_id: &str,
