...
fn build_admin_provider_model_response(
    model: &StoredAdminProviderModel,
    now_unix_secs: u64,
) -> serde_json::Value {
...
async fn build_admin_provider_models_payload(
    state: &AppState,
    provider_id: &str,
    skip: usize,
    limit: usize,
...
async fn build_admin_provider_model_payload(
    state: &AppState,
    provider_id: &str,
    model_id: &str,
) -> Option<serde_json::Value> {
    ...
    let model = state.get_admin_provider_model(provider_id, model_id).await.ok()??;
    let now_unix_secs = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .ok()
        .map(|duration| duration.as_secs())
...
async fn admin_provider_model_name_exists(
    state: &AppState,
    provider_id: &str,
...
include!("models/global.rs");
include!("models/external_cache.rs");
include!("models/routing.rs");
include!("models/write.rs");
