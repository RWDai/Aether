...
fn parse_admin_provider_oauth_batch_import_request(
    ...
    match serde_json::from_slice::<AdminProviderOAuthBatchImportRequest>(request_body) {
        ...
        _ => Err(build_internal_hub_error_response(
            http::StatusCode::BAD_REQUEST,
            "请求体必须是合法的 JSON 对象",
        )),
    }
...
fn parse_admin_provider_oauth_batch_import_entries(
    ...
    if raw.starts_with('{') {
        if let Ok(value @ serde_json::Value::Object(_)) = serde_json::from_str::<serde_json::Value>(raw)
        {
...
async fn extract_admin_provider_oauth_batch_error_detail(
...
async fn handle_admin_provider_oauth_batch_import(
    state: &AppState,
    ...
    let total = estimate_admin_provider_oauth_batch_import_total(&provider_type, payload.credentials.as_str());
    if total == 0 {
        return Ok(build_internal_hub_error_response(
            http::StatusCode::BAD_REQUEST,
            "未找到有效的 Token 数据",
...
async fn handle_admin_provider_oauth_start_batch_import_task(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    request_body: Option<&axum::body::Bytes>,
) -> Result<Response<Body>, GatewayError> {
        ...
        .await
        {
            Ok(outcome) => {
                ...
                let error_samples = outcome
                    .results
                    .iter()
                    .filter(|item| item.get("status").and_then(serde_json::Value::as_str) == Some("error"))
                    .take(PROVIDER_OAUTH_BATCH_TASK_MAX_ERROR_SAMPLES)
                    .cloned()
                    .collect::<Vec<_>>();
                let message = format!("导入完成：成功 {}，失败 {}", outcome.success, outcome.failed);
