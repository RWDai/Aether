async fn handle_admin_provider_oauth_complete_key(
    state: &AppState,
    ...
    if !state_data.provider_type.trim().is_empty()
        && !state_data.provider_type.eq_ignore_ascii_case(&provider_type)
    {
        return Ok(build_internal_hub_error_response(
            http::StatusCode::BAD_REQUEST,
            "state 无效或已过期",
            ...
            .is_some_and(|value| !value.is_empty())
    {
        ...
        if let Some(endpoint) = endpoints.into_iter().find(|endpoint| {
            endpoint.is_active && endpoint.api_format.trim().eq_ignore_ascii_case("openai:cli")
        }) {
...
async fn handle_admin_provider_oauth_complete_provider(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    request_body: Option<&axum::body::Bytes>,
) -> Result<Response<Body>, GatewayError> {
    ...
    let duplicate = match find_duplicate_provider_oauth_key(state, &provider_id, &auth_config, None)
        .await
    {
