...
async fn handle_admin_provider_oauth_device_authorize(
    state: &AppState,
    ...
    let Some(provider_id) =
        ...
        return Ok(build_internal_hub_error_response(
    ...
    let payload = match serde_json::from_slice::<AdminProviderOAuthDeviceAuthorizePayload>(request_body)
    {
        Ok(payload) => payload,
        Err(_) => {
            return Ok(build_internal_hub_error_response(
    ...
    let region = normalize_kiro_device_region(Some(payload.region.as_str()))
        .ok_or_else(|| {
            build_internal_hub_error_response(http::StatusCode::BAD_REQUEST, "region 格式无效")
        });
...
async fn handle_admin_provider_oauth_device_poll(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    request_body: Option<&axum::body::Bytes>,
) -> Result<Response<Body>, GatewayError> {
    ...
    let Some(provider_id) = admin_provider_oauth_device_poll_provider_id(&request_context.request_path)
    else {
        return Ok(build_internal_hub_error_response(
            http::StatusCode::NOT_FOUND,
            "Provider 不存在",
    ...
    auth_config.insert("client_secret".to_string(), json!(session.client_secret.clone()));
    ...
    let duplicate = match find_duplicate_provider_oauth_key(state, &provider_id, &auth_config, None).await
    {
    ...
    let _ =
        refresh_provider_oauth_account_state_after_update(state, &provider, &persisted_key.id).await;
