async fn handle_admin_provider_oauth_start_key(
    state: &AppState,
    ...
    let pkce_verifier = template.use_pkce.then(generate_provider_oauth_pkce_verifier);
...
async fn handle_admin_provider_oauth_start_provider(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
) -> Result<Response<Body>, GatewayError> {
