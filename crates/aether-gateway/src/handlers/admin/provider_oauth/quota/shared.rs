fn provider_auto_remove_banned_keys(config: Option<&serde_json::Value>) -> bool {
    config
...
fn should_auto_remove_structured_reason(reason: Option<&str>) -> bool {
    reason
        .map(str::trim)
        .is_some_and(|value| value.starts_with(OAUTH_ACCOUNT_BLOCK_PREFIX))
}

fn normalize_string_id_list(values: Option<Vec<String>>) -> Option<Vec<String>> {
    let mut out = Vec::new();
...
fn coerce_json_u64(value: &serde_json::Value) -> Option<u64> {
    match value {
...
fn coerce_json_f64(value: &serde_json::Value) -> Option<f64> {
    match value {
...
fn extract_execution_error_message(result: &ExecutionResult) -> Option<String> {
...
fn quota_refresh_success_invalid_state(
...
fn coerce_json_string(value: Option<&serde_json::Value>) -> Option<String> {
    value
...
async fn persist_provider_quota_refresh_state(
    state: &AppState,
    key_id: &str,
...
async fn execute_provider_quota_plan(
    state: &AppState,
        ...
        .await
    {
        ...
        Err(err) => {
            warn!(
                ...
                "gateway provider quota executor request failed"
            );
            return Ok(None);
        }
    };
    ...
    if response.status() != http::StatusCode::OK {
        warn!(
            ...
            quota_kind = %quota_kind,
            "gateway provider quota executor returned non-200"
        );
        return Ok(None);
    }
