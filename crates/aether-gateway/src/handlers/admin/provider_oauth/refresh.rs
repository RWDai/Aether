fn build_internal_hub_error_response(
...
fn normalize_provider_oauth_refresh_error_message(
...
fn merge_provider_oauth_refresh_failure_reason(
...
fn provider_oauth_key_proxy_value(proxy_node_id: Option<&str>) -> Option<serde_json::Value> {
    proxy_node_id
        .map(str::trim)
        .filter(|value| !value.is_empty())
...
fn provider_oauth_active_api_formats(endpoints: &[StoredProviderCatalogEndpoint]) -> Vec<String> {
...
async fn find_duplicate_provider_oauth_key(
    state: &AppState,
    provider_id: &str,
...
fn build_provider_oauth_auth_config_from_token_payload(
    provider_type: &str,
...
async fn create_provider_oauth_catalog_key(
    state: &AppState,
    provider_id: &str,
    name: &str,
    access_token: &str,
...
async fn update_existing_provider_oauth_catalog_key(
    state: &AppState,
...
fn provider_oauth_runtime_endpoint_for_provider(
    provider_type: &str,
...
async fn refresh_provider_oauth_account_state_after_update(
    state: &AppState,
