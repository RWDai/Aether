include!("providers_helpers/summary.rs");
include!("providers_helpers/pool.rs");

async fn run_admin_provider_delete_task(
    state: &AppState,
    provider_id: &str,
    task_id: &str,
...
fn public_global_model_mapping_patterns(model: &StoredPublicGlobalModel) -> Vec<String> {
    model
        .config
        .as_ref()
...
fn mapping_preview_masked_catalog_api_key(
    state: &AppState,
...
async fn build_admin_provider_mapping_preview_payload(
    state: &AppState,
    provider_id: &str,
