async fn build_admin_providers_payload(
    state: &AppState,
    skip: usize,
    limit: usize,
...
fn build_admin_provider_summary_value(
...
async fn build_admin_provider_summary_payload(
    state: &AppState,
    provider_id: &str,
) -> Option<serde_json::Value> {
    if !state.has_provider_catalog_data_reader() {
...
async fn build_admin_providers_summary_payload(
    state: &AppState,
    page: usize,
    page_size: usize,
    search: &str,
...
async fn build_admin_provider_health_monitor_payload(
    state: &AppState,
    provider_id: &str,
    lookback_hours: u64,
