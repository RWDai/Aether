...
fn mask_admin_proxy_node_password(password: Option<&str>) -> Option<String> {
    ...
    Some(format!("{}****{}", &password[..2], &password[password.len() - 2..]))
}
...
fn build_admin_proxy_node_payload(node: &StoredProxyNode) -> serde_json::Value {
    ...
    if node.is_manual {
        ...
        payload.insert(
            ...
            json!(mask_admin_proxy_node_password(node.proxy_password.as_deref())),
        );
    }
...
async fn maybe_build_local_admin_proxy_nodes_response(
    state: &AppState,
        ...
        )
    {
        if !state.data.has_proxy_node_reader() {
            return Ok(Some(
                build_admin_proxy_nodes_maintenance_response(),
            ));
        }
