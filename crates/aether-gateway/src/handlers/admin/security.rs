...
async fn build_admin_security_blacklist_add_response(
    ...
    let Some(request_body) = request_body else {
        return Ok(build_admin_security_bad_request_response("请求数据验证失败"));
    };
    let payload = match serde_json::from_slice::<AdminSecurityBlacklistAddRequest>(request_body) {
        Ok(value) if !value.ip_address.trim().is_empty() && !value.reason.trim().is_empty() => value,
        _ => return Ok(build_admin_security_bad_request_response("请求数据验证失败")),
    };
...
async fn build_admin_security_whitelist_add_response(
    ...
    let payload = match serde_json::from_slice::<AdminSecurityWhitelistAddRequest>(request_body) {
        Ok(value) => value,
        Err(_) => return Ok(build_admin_security_bad_request_response("请求数据验证失败")),
    };
...
async fn maybe_build_local_admin_security_response(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    request_body: Option<&axum::body::Bytes>,
