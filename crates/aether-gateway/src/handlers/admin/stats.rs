...
impl AdminStatsTimeRange {
    fn resolve_optional(query: Option<&str>) -> Result<Option<Self>, String> {
        ...
        let (start_date, end_date) = match (preset.as_deref(), start_date, end_date) {
            ...
            _ => {
                return Err("Either preset or both start_date and end_date must be provided"
                    .to_string());
            }
        };
    ...
    fn to_unix_bounds(&self) -> Option<(u64, u64)> {
        ...
        let end_local = self
            ...
            .and_hms_opt(0, 0, 0)?;
        let start_utc = chrono::DateTime::<Utc>::from_naive_utc_and_offset(start_local - offset, Utc)
            .timestamp();
        let end_utc = chrono::DateTime::<Utc>::from_naive_utc_and_offset(end_local - offset, Utc)
            .timestamp();
    ...
    fn local_date_for_unix_secs(&self, unix_secs: u64) -> Option<chrono::NaiveDate> {
        ...
        let local = timestamp.checked_add_signed(chrono::Duration::minutes(i64::from(
...
async fn maybe_build_local_admin_stats_response(
    state: &AppState,
        ...
        )
    {
        ...
        return Ok(Some(Json(json!({ "providers": payload })).into_response()));
    }
        ...
        )
    {
        ...
        for item in usage {
            let Some(category) = item.error_category.as_ref().filter(|value| !value.trim().is_empty()) else {
                continue;
            };
        ...
        distribution_items.sort_by(|left, right| {
            let left_count = left.get("count").and_then(serde_json::Value::as_u64).unwrap_or(0);
            let right_count = right.get("count").and_then(serde_json::Value::as_u64).unwrap_or(0);
            right_count
                .cmp(&left_count)
                .then_with(|| {
                    left.get("category")
        ...
        )
    {
        ...
        let usage =
            list_usage_for_range(state, &time_range, &AdminStatsUsageFilter::default()).await?;
        let mut by_day: std::collections::BTreeMap<String, (Vec<u64>, Vec<u64>)> =
            time_range
                .local_date_strings()
                .into_iter()
        ...
        return Ok(Some(Json(serde_json::Value::Array(payload)).into_response()));
    }

    if decision.route_kind.as_deref() == Some("cost_forecast")
        && request_context.request_method == http::Method::GET
        && matches!(
            request_context.request_path.as_str(),
    ...
    if decision.route_kind.as_deref() == Some("cost_savings")
        && request_context.request_method == http::Method::GET
        && matches!(
            request_context.request_path.as_str(),
        ...
        )
    {
        ...
        let usage = list_usage_for_range(state, time_range.as_ref().expect("time range exists"), &filters)
            .await?;
    ...
    if decision.route_kind.as_deref() == Some("leaderboard_models")
        && request_context.request_method == http::Method::GET
        && matches!(
            request_context.request_path.as_str(),
    ...
    if decision.route_kind.as_deref() == Some("leaderboard_api_keys")
        && request_context.request_method == http::Method::GET
        && matches!(
            request_context.request_path.as_str(),
        ...
        )
    {
        ...
        let snapshots = if state.has_auth_api_key_data_reader() {
            let api_key_ids: Vec<String> = usage
                ...
                .into_iter()
                .collect();
            Some(state.read_auth_api_key_snapshots_by_ids(&api_key_ids).await?)
        } else {
            None
        };
    ...
    if decision.route_kind.as_deref() == Some("leaderboard_users")
        && request_context.request_method == http::Method::GET
        && matches!(
            request_context.request_path.as_str(),
        ...
        )
    {
        ...
        let filters = AdminStatsUsageFilter::from_query(
            request_context.request_query_string.as_deref(),
        );
...
fn admin_stats_provider_quota_usage_empty_response() -> Response<Body> {
    Json(json!({
        "providers": [],
...
fn admin_stats_cost_forecast_empty_response() -> Response<Body> {
    Json(json!({
        "history": [],
        "forecast": [],
        "slope": 0.0,
...
fn admin_stats_comparison_empty_response(
...
fn admin_stats_error_distribution_empty_response() -> Response<Body> {
    Json(json!({
        "distribution": [],
        "trend": [],
    }))
...
fn admin_stats_performance_percentiles_empty_response() -> Response<Body> {
    Json(json!([])).into_response()
}

fn admin_stats_cost_savings_empty_response() -> Response<Body> {
    Json(json!({
        "cache_read_tokens": 0,
        "cache_read_cost": 0.0,
...
fn admin_stats_leaderboard_empty_response(
...
fn admin_stats_time_series_empty_response() -> Response<Body> {
    Json(json!([])).into_response()
}

fn admin_stats_bad_request_response(detail: String) -> Response<Body> {
    (
...
fn parse_tz_offset_minutes(query: Option<&str>) -> Result<i32, String> {
...
fn parse_naive_date(field: &str, value: &str) -> Result<chrono::NaiveDate, String> {
    chrono::NaiveDate::parse_from_str(value, "%Y-%m-%d")
        .map_err(|_| format!("{field} must be a valid date in YYYY-MM-DD format"))
}

fn parse_bounded_u32(field: &str, value: &str, min: u32, max: u32) -> Result<u32, String> {
    let parsed = value
        .parse::<u32>()
...
fn parse_nonnegative_usize(field: &str, value: &str) -> Result<usize, String> {
    value
        .parse::<usize>()
        .map_err(|_| format!("{field} must be a valid integer"))
}

fn admin_usage_default_days() -> usize {
    match std::env::var("ADMIN_USAGE_DEFAULT_DAYS") {
        Ok(value) => value.parse::<usize>().ok().unwrap_or(0),
        Err(_) => match std::env::var("ENVIRONMENT") {
            Ok(value)
                if !matches!(value.as_str(), "development" | "test" | "testing") =>
            {
                30
            }
            _ => 0,
...
fn user_today(tz_offset_minutes: i32) -> chrono::NaiveDate {
    (Utc::now() + chrono::Duration::minutes(i64::from(tz_offset_minutes))).date_naive()
}

fn resolve_preset_dates(
    preset: &str,
    tz_offset_minutes: i32,
    ...
    match preset {
        ...
        "this_week" => {
            let week_start = user_today
                .checked_sub_signed(chrono::Duration::days(i64::from(user_today.weekday().num_days_from_monday())))
                .unwrap_or(user_today);
        ...
        "this_year" => {
            let start = chrono::NaiveDate::from_ymd_opt(user_today.year(), 1, 1)
                .unwrap_or(user_today);
            Ok((start, user_today))
        }
...
async fn list_usage_for_range(
    ...
    let Some((created_from_unix_secs, created_until_unix_secs)) = time_range.to_unix_bounds() else {
        return Ok(Vec::new());
    };
...
async fn list_usage_for_optional_range(
    state: &AppState,
    time_range: Option<&AdminStatsTimeRange>,
    filters: &AdminStatsUsageFilter,
) -> Result<Vec<aether_data::repository::usage::StoredRequestUsageAudit>, GatewayError> {
...
fn build_model_leaderboard_items(
...
fn build_api_key_leaderboard_items(
    items: &[aether_data::repository::usage::StoredRequestUsageAudit],
...
fn build_user_leaderboard_items(
    items: &[aether_data::repository::usage::StoredRequestUsageAudit],
...
async fn load_user_leaderboard_metadata(
    state: &AppState,
    user_ids: &[String],
...
fn build_daily_time_series_buckets(
    time_range: &AdminStatsTimeRange,
    items: &[aether_data::repository::usage::StoredRequestUsageAudit],
...
fn build_hourly_time_series_payload(
    ...
    for item in items {
        let Some(unix_secs) = i64::try_from(item.created_at_unix_secs).ok()
        else {
            continue;
        };
...
fn percentile_cont(values: &mut [u64], percentile: f64) -> Option<u64> {
...
fn pct_change_value(current: f64, previous: f64) -> serde_json::Value {
    if previous == 0.0 {
        if current == 0.0 {
            json!(0.0)
        } else {
