async fn maybe_build_local_internal_proxy_response(
    state: &AppState,
    ...
    maybe_build_local_internal_proxy_response_impl(
        state,
        request_context,
        remote_addr,
        request_body,
...
async fn maybe_build_local_admin_proxy_response(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    request_body: Option<&axum::body::Bytes>,
) -> Result<Option<Response<Body>>, GatewayError> {
    ...
    if let Some(response) =
        maybe_build_local_admin_provider_oauth_response(state, request_context, request_body)
            .await?
    {
    ...
    if let Some(response) =
        maybe_build_local_admin_announcements_response(state, request_context, request_body)
            .await?
    {
        return Ok(Some(response));
    }
    ...
    if let Some(response) =
        maybe_build_local_admin_core_response(state, request_context, request_body).await?
    {
        return Ok(Some(response));
    }

    if let Some(response) =
        maybe_build_local_admin_global_models_response(state, request_context, request_body).await?
    {
        return Ok(Some(response));
    }

    if let Some(response) =
        maybe_build_local_admin_provider_models_response(state, request_context, request_body)
            .await?
    {
        return Ok(Some(response));
    }
    ...
    if let Some(response) =
        maybe_build_local_admin_providers_response(state, request_context, request_body).await?
    {
        return Ok(Some(response));
    }
    if let Some(response) =
    ...
    if let Some(response) =
        maybe_build_local_admin_endpoints_response(state, request_context, request_body).await?
    {
        return Ok(Some(response));
    }
