...
fn request_candidate_event_unix_secs(candidate: &StoredRequestCandidate) -> u64 {
    candidate
...
fn normalize_admin_base_url(base_url: &str) -> Result<String, String> {
...
fn admin_requested_force_stream(value: &serde_json::Value) -> bool {
    match value {
...
#[derive(Clone, Copy)]
struct ApiFormatHealthMonitorOptions {
    include_api_path: bool,
    include_provider_count: bool,
    include_key_count: bool,
}

fn provider_key_api_formats(key: &StoredProviderCatalogKey) -> Vec<String> {
    key.api_formats
        .as_ref()
...
async fn build_public_providers_payload(
    state: &AppState,
    query: Option<&str>,
...
async fn build_public_catalog_models_payload(
    state: &AppState,
    query: Option<&str>,
) -> Option<serde_json::Value> {
...
async fn build_public_catalog_search_models_payload(
    state: &AppState,
    query: Option<&str>,
) -> Option<serde_json::Value> {
    if !state.has_global_model_data_reader() {
...
async fn build_api_format_health_monitor_payload(
    state: &AppState,
    lookback_hours: u64,
...
fn build_public_health_timeline(
...
fn api_format_display_name(api_format: &str) -> String {
