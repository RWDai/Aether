include!("support/announcements.rs");
include!("support/auth.rs");
include!("support/dashboard.rs");
include!("support/monitoring.rs");
include!("support/models.rs");
include!("support/test_connection.rs");
include!("support/user_me.rs");
include!("support/wallet.rs");
include!("support/payment.rs");
...
async fn maybe_build_local_public_support_response(
    state: &AppState,
    ...
    if decision.route_family.as_deref() == Some("auth_legacy") {
        return maybe_build_local_auth_legacy_response(state, request_context, headers, request_body)
            .await;
    }
    ...
    if decision.route_family.as_deref() == Some("announcement_user_legacy") {
        return maybe_build_local_announcement_user_response(state, request_context, headers).await;
    }
    ...
    if decision.route_family.as_deref() == Some("announcements")
    {
            ...
            )
        {
            ...
            return Some(Json(build_public_announcement_list_payload(page)).into_response());
        }

        if decision.route_kind.as_deref() == Some("active")
            && matches!(
                request_context.request_path.as_str(),
        ...
        if decision.route_kind.as_deref() != Some("detail") {
            return None;
        }
        ...
        return Some(Json(build_public_announcement_payload(&announcement)).into_response());
    }
