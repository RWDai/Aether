...
async fn maybe_build_local_dashboard_legacy_response(
    state: &AppState,
...
fn decision_route_kind(request_context: &GatewayPublicRequestContext) -> Option<&str> {
    request_context
        .control_decision
        .as_ref()
...
fn dashboard_resolve_preset_dates(
    ...
    match preset {
        ...
        "last_month" => {
            ...
            Ok((last_month_end.with_day(1).unwrap_or(last_month_end), last_month_end))
        }
        "this_year" => Ok((
...
async fn dashboard_list_usage_for_range(
    ...
    let Some((created_from_unix_secs, created_until_unix_secs)) = dashboard_range_bounds_unix(range)
    else {
...
async fn dashboard_load_api_key_counts(
    ...
    let summary = if is_admin {
        ...
        state
            .summarize_auth_api_key_export_records_by_user_ids(&[user_id.to_string()], now_unix_secs)
            .await?
    };
...
async fn handle_dashboard_stats_get(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    headers: &http::HeaderMap,
) -> Response<Body> {
    ...
    let api_key_counts = match dashboard_load_api_key_counts(
        state,
        is_admin,
        &auth.user.id,
    )
    ...
    if is_admin {
        ...
        let success_rate = if today_totals.requests == 0 {
            ...
            (today_totals.requests.saturating_sub(today_totals.error_requests)) as f64
                / today_totals.requests as f64
                * 100.0
        };
        let cost_savings =
            dashboard_round_f64(period_totals.total_cost_usd - period_totals.actual_total_cost_usd, 4);
        let stats = json!([
            {
                "name": "今日请求",
...
async fn handle_dashboard_daily_stats_get(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    headers: &http::HeaderMap,
) -> Response<Body> {
    ...
    let mut by_date = std::collections::BTreeMap::<chrono::NaiveDate, DashboardDailyAggregate>::new();
    let mut model_summary = std::collections::BTreeMap::<String, DashboardModelAggregate>::new();
    let mut provider_summary = std::collections::BTreeMap::<String, DashboardProviderAggregate>::new();
    ...
    for item in &usage {
        ...
        let provider = provider_summary.entry(item.provider_name.clone()).or_default();
        provider.requests += 1;
        provider.tokens += item.total_tokens;
        provider.cost += item.total_cost_usd;
    }
...
async fn handle_dashboard_recent_requests_get(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    headers: &http::HeaderMap,
) -> Response<Body> {
    ...
    let usage = match state
        .list_recent_usage_audits(
            (!is_admin).then_some(auth.user.id.as_str()),
            limit,
        )
        .await
    {
...
async fn handle_dashboard_provider_status_get(
    state: &AppState,
    request_context: &GatewayPublicRequestContext,
    headers: &http::HeaderMap,
) -> Response<Body> {
    ...
    match query_param_value(query, field) {
        Some(value) => {
            ...
            if (min..=max).contains(&parsed) {
                ...
                Err(format!("{field} must be an integer between {min} and {max}"))
            }
        }
        None => Ok(default),
    }
