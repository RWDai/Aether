...
struct AdminSetModuleEnabledRequest {
    enabled: bool,
}

struct AdminModuleRuntimeState {
...
fn oauth_module_config_is_valid(
...
fn ldap_module_config_is_valid(
...
async fn build_public_auth_modules_status_payload(
    state: &AppState,
...
fn admin_module_by_name(name: &str) -> Option<&'static AdminModuleDefinition> {
    ADMIN_MODULE_DEFINITIONS
        .iter()
        .find(|module| module.name == name)
}

async fn build_admin_module_runtime_state(
    state: &AppState,
...
fn build_admin_module_validation_result(
...
async fn build_admin_module_status_payload(
    state: &AppState,
    module: &AdminModuleDefinition,
    runtime: &AdminModuleRuntimeState,
) -> Result<serde_json::Value, GatewayError> {
...
async fn build_admin_modules_status_payload(
    state: &AppState,
) -> Result<serde_json::Value, GatewayError> {
...
fn admin_module_name_from_status_path(request_path: &str) -> Option<String> {
    request_path
...
fn admin_module_name_from_enabled_path(request_path: &str) -> Option<String> {
    request_path
        .strip_prefix("/api/admin/modules/status/")
