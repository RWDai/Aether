...
fn module_available_from_env(env_key: &str, default_available: bool) -> bool {
...
fn system_config_bool(value: Option<&serde_json::Value>, default: bool) -> bool {
    match value {
...
fn system_config_string(value: Option<&serde_json::Value>) -> Option<String> {
    match value {
        Some(serde_json::Value::String(value)) => {
...
fn current_aether_version() -> String {
    let version_file =
...
fn build_admin_system_check_update_payload() -> serde_json::Value {
    json!({
...
async fn build_admin_system_stats_payload(
    state: &AppState,
    ...
    let active_providers = providers.iter().filter(|provider| provider.is_active).count() as u64;
...
async fn build_admin_system_settings_payload(
    state: &AppState,
) -> Result<serde_json::Value, GatewayError> {
...
async fn apply_admin_system_settings_update(
    state: &AppState,
...
fn build_admin_api_formats_payload() -> serde_json::Value {
    json!({
...
fn admin_email_template_definition(template_type: &str) -> Option<&'static AdminEmailTemplateDefinition>
{
...
async fn read_admin_email_template_payload(
    state: &AppState,
    template_type: &str,
...
async fn build_admin_email_templates_payload(
    state: &AppState,
) -> Result<serde_json::Value, GatewayError> {
    ...
    for definition in ADMIN_EMAIL_TEMPLATE_DEFINITIONS {
        if let Some(payload) = read_admin_email_template_payload(state, definition.template_type).await?
        {
...
async fn build_admin_email_template_payload(
    state: &AppState,
    template_type: &str,
) -> Result<Result<serde_json::Value, (http::StatusCode, serde_json::Value)>, GatewayError> {
...
fn escape_admin_email_template_html(value: &str) -> String {
    value
        .replace('&', "&amp;")
        .replace('<', "&lt;")
        .replace('>', "&gt;")
...
fn render_admin_email_template_html(
    template_html: &str,
...
async fn apply_admin_email_template_update(
    state: &AppState,
    template_type: &str,
    request_body: &axum::body::Bytes,
) -> Result<Result<serde_json::Value, (http::StatusCode, serde_json::Value)>, GatewayError> {
...
async fn preview_admin_email_template(
    state: &AppState,
    template_type: &str,
...
async fn reset_admin_email_template(
    state: &AppState,
    template_type: &str,
) -> Result<Result<serde_json::Value, (http::StatusCode, serde_json::Value)>, GatewayError> {
    let Some(definition) = admin_email_template_definition(template_type) else {
...
fn decrypt_admin_system_export_secret(state: &AppState, ciphertext: &str) -> Option<String> {
    decrypt_catalog_secret_with_fallbacks(state.encryption_key(), ciphertext)
}

fn normalize_admin_system_export_api_formats(
    ...
    for raw in raw_formats {
        let Some(value) = raw.as_str().map(str::trim).filter(|value| !value.is_empty()) else {
            continue;
        };
...
fn resolve_admin_system_export_key_api_formats(
    raw_formats: Option<&serde_json::Value>,
...
fn collect_admin_system_export_provider_endpoint_formats(
...
fn decrypt_admin_system_export_provider_config(
    state: &AppState,
...
async fn build_admin_system_config_export_payload(
    state: &AppState,
) -> Result<serde_json::Value, GatewayError> {
    ...
    let provider_ids = providers.iter().map(|provider| provider.id.clone()).collect::<Vec<_>>();
    let endpoints = state
...
fn serialize_admin_system_users_export_wallet(
...
fn build_admin_system_users_export_api_key_payload(
    ...
    let mut payload = serde_json::Map::from_iter([
        ...
        ("allowed_models".to_string(), json!(key.allowed_models.clone())),
        ("rate_limit".to_string(), json!(key.rate_limit)),
        (
            "concurrent_limit".to_string(),
            json!(key.concurrent_limit),
        ),
        (
...
async fn build_admin_system_users_export_payload(
    state: &AppState,
) -> Result<serde_json::Value, GatewayError> {
    ...
    let wallets_by_api_key_id = standalone_wallets
        .into_iter()
        .filter_map(|wallet| wallet.api_key_id.clone().map(|api_key_id| (api_key_id, wallet)))
        .collect::<BTreeMap<_, _>>();
    ...
    let users_data = users
        ...
        .map(|user| {
            ...
            let api_keys_payload = api_keys
                .iter()
                .map(|key| {
                    build_admin_system_users_export_api_key_payload(state, key, None, true)
                })
                .collect::<Vec<_>>();
...
fn build_admin_system_configs_payload(
    entries: &[crate::gateway::data::StoredSystemConfigEntry],
) -> serde_json::Value {
    let has_request_record_level = entries.iter().any(|entry| entry.key == REQUEST_RECORD_LEVEL_KEY);
    json!(
        entries
            .iter()
            .filter_map(|entry| {
...
async fn build_admin_system_config_detail_payload(
    state: &AppState,
    requested_key: &str,
) -> Result<Result<serde_json::Value, (http::StatusCode, serde_json::Value)>, GatewayError> {
...
async fn apply_admin_system_config_update(
    state: &AppState,
    requested_key: &str,
    request_body: &axum::body::Bytes,
) -> Result<Result<serde_json::Value, (http::StatusCode, serde_json::Value)>, GatewayError> {
    ...
    {
        let Some(encryption_key) = state.encryption_key().filter(|value| !value.trim().is_empty())
        else {
            return Ok(Err((
        ...
        value = json!(
            encrypt_python_fernet_plaintext(encryption_key, plaintext)
                .map_err(|err| GatewayError::Internal(err.to_string()))?
        );
    }
...
async fn delete_admin_system_config(
    state: &AppState,
    requested_key: &str,
) -> Result<Result<serde_json::Value, (http::StatusCode, serde_json::Value)>, GatewayError> {
