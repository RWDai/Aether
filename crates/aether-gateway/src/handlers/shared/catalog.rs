fn decrypt_catalog_secret_with_fallbacks(
...
fn effective_catalog_encryption_key(state: &AppState) -> Option<String> {
...
fn encrypt_catalog_secret_with_fallbacks(state: &AppState, plaintext: &str) -> Option<String> {
    let encryption_key = effective_catalog_encryption_key(state)?;
    encrypt_python_fernet_plaintext(&encryption_key, plaintext).ok()
}

fn masked_catalog_api_key(state: &AppState, key: &StoredProviderCatalogKey) -> String {
...
fn parse_catalog_auth_config_json(
    state: &AppState,
...
fn default_provider_key_status_snapshot() -> serde_json::Value {
    json!({
        "oauth": {
            "code": "none",
...
fn provider_key_status_snapshot_payload(key: &StoredProviderCatalogKey) -> serde_json::Value {
    key.status_snapshot
        .clone()
...
fn provider_key_health_summary(
    key: &StoredProviderCatalogKey,
) -> (
    f64,
    i64,
...
fn build_admin_provider_key_response(
    state: &AppState,
    key: &StoredProviderCatalogKey,
    now_unix_secs: u64,
) -> serde_json::Value {
