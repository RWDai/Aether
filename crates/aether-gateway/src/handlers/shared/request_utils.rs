fn rust_auth_terminates_provider_credentials(decision: Option<&GatewayControlDecision>) -> bool {
    decision.is_some_and(|decision| {
        decision.route_class.as_deref() == Some("ai_public") && decision.auth_context.is_some()
    })
}

fn mark_external_models_official_providers(value: &serde_json::Value) -> Option<serde_json::Value> {
...
fn should_strip_forwarded_provider_credential_header(
...
fn should_strip_forwarded_trusted_admin_header(
    decision: Option<&GatewayControlDecision>,
    header_name: &HeaderName,
) -> bool {
...
fn sanitize_upstream_path_and_query(
    decision: Option<&GatewayControlDecision>,
...
fn strip_query_param(path_and_query: &str, key_to_strip: &str) -> String {
...
fn query_param_optional_bool(query: Option<&str>, key: &str) -> Option<bool> {
    let query = query?;
    for (entry_key, value) in form_urlencoded::parse(query.as_bytes()) {
        if entry_key == key {
            let normalized = value.trim().to_ascii_lowercase();
...
fn request_enables_control_execute(headers: &http::HeaderMap) -> bool {
    [
...
fn admin_proxy_local_requires_buffered_body(request_context: &GatewayPublicRequestContext) -> bool {
    request_context
        .control_decision
        .as_ref()
        .is_some_and(|decision| {
            ...
            match (
                ...
                | (
                    Some("provider_ops_manage"),
                    http::Method::POST,
                    Some("verify_provider"),
                )
                | (
                    Some("provider_ops_manage"),
                    http::Method::PUT,
                    Some("save_provider_config"),
                )
                | (
                ...
                | (
                    Some("adaptive_manage"),
                    http::Method::PATCH,
                    Some("toggle_mode"),
                )
                | (
                    Some("security_manage"),
                    http::Method::POST,
                    Some("blacklist_add"),
                )
                | (
                ...
                | (
                    Some("users_manage"),
                    http::Method::POST,
                    Some("create_user_api_key"),
                )
                | (
                    Some("users_manage"),
                    http::Method::PUT,
                    Some("update_user_api_key"),
                )
                | (
                    Some("users_manage"),
                    http::Method::PATCH,
                    Some("lock_user_api_key"),
                )
                | (
                    Some("pool_manage"),
                    http::Method::POST,
...
fn internal_proxy_local_requires_buffered_body(
    ...
    request_context
        ...
        .is_some_and(|decision| {
            ...
            matches!(
                ...
                (Some("hub_manage"), Some("heartbeat" | "node_status"))
                    | (
                        Some("gateway_legacy"),
                        Some(
                            "resolve"
...
fn public_support_local_requires_buffered_body(
    request_context: &GatewayPublicRequestContext,
) -> bool {
    request_context
        .control_decision
...
fn local_proxy_route_requires_buffered_body(request_context: &GatewayPublicRequestContext) -> bool {
