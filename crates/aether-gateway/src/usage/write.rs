...
pub(crate) fn build_sync_terminal_usage_event(
    ...
    let mut data = UsageEventData {
        ...
        response_headers: Some(headers_to_json(&payload.headers)),
        ...
        client_response_body: payload.client_body_json.clone(),
        error_message: resolve_error_message(
            payload.status_code,
            payload.body_json.as_ref(),
            payload.body_base64.as_deref(),
        ),
        error_category: resolve_error_category(payload.status_code, event_type),
        ..UsageEventData::default()
    };

    let standardized_response_body = payload
        .body_json
        .as_ref()
        .cloned()
        .or_else(|| data.response_body.clone());
    if let Some(tokens) = standardized_response_body.as_ref() {
        apply_standardized_usage(
            resolve_billing_api_format(plan, report_context),
            tokens,
            &mut data,
        );
    }
    if data.total_tokens.is_none() {
        if let Some(tokens) = payload
            .body_json
            .as_ref()
            .and_then(extract_token_counts_from_json)
            .or_else(|| {
                payload
                    .body_base64
                    .as_deref()
                    .and_then(extract_token_counts_from_base64_payload)
            })
        {
    ...
    build_usage_event(plan, report_context, data, event_type)
}
...
pub(crate) fn build_stream_terminal_usage_event(
    ...
    let mut data = UsageEventData {
        ...
        response_headers: Some(headers_to_json(&payload.headers)),
        client_response_headers: Some(headers_to_json(&payload.headers)),
        response_body: decode_body_for_storage(payload.body_base64.as_deref()),
        error_message: resolve_error_message(
            payload.status_code,
            None,
            payload.body_base64.as_deref(),
    ...
    let standardized_response_body = data.response_body.clone();
    if let Some(response_body) = standardized_response_body.as_ref() {
        apply_standardized_usage(
            resolve_billing_api_format(plan, report_context),
            response_body,
            &mut data,
...
#[cfg(test)]
mod tests {
    use super::{build_upsert_usage_record_from_event, extract_token_counts_from_json};
